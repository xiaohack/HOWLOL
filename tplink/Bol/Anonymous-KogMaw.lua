...
function ScriptMsg(msg)
  print("<font color=\"#daa520\"><b>Anonymous KogMaw:</b></font> <font color=\"#FFFFFF\">"..msg.."</font>")
end
...
local _Combo = {"Combo", "콤보", "组合"}
local _Harass = {"Harass", "괴롭히기", "骚扰"}
local _LineClear = {"LineClear", "라인클리어", "线清晰"}
local _KillSteal = {"KillSteal","죽여","击杀选项"}
local _Draw = {"Draw","무승부","绘制"}
local _WSetting = {"WSetting","W환경","W设定"}
local _RSetting = {"RSetting","R환경","R设定"}

local UseQ = {"Use Q", "Q 사용", "用 Q"}
local UseW = {"Use W", "W 사용", "用 W"}
local UseE = {"Use E", "E 사용", "用 E"}
local UseR = {"Use R", "R 사용", "用 R"}
local DrawQRange = {"DrawQRange", "Q 범위", "Q范围"}
local DrawQColor = {"DrawQColor", "Q 색상의 범위", "Q范围颜色"}
local DrawWRange = {"DrawWRange", "W 범위", "W范围"}
local DrawERange = {"DrawERange", "E 범위", "E范围"}
local DrawRRange = {"DrawRRange", "R 범위", "R范围"}
local DrawWColor = {"DrawWColor", "W 색상의 범위", "W范围颜色"}
local DrawEColor = {"DrawEColor", "E 색상의 범위", "E范围颜色"}
local DrawRColor = {"DrawRColor", "R 색상의 범위", "R范围颜色"}
local UseWinAARange = {"UseWinAARange", "W배터를 사용하는 경우", "连击时使用W"}
local PriorityWtarget = {"PriorityWtarget", "우선 목표 W", "优先使用W"}
local UseRLimit = {"UseRLimit", "사용 제한 R", "R的使用范围"}
local LimitRStack = {"LimitRStack", "제한 R 스택", "限制R的堆叠层数"}
local UseRDelay = {"UseRDelay", "R 사용 지연", "使用R延迟"}
local DelayRChoice = {"DelayRChoice", "지연 R은 선택", "延迟R选择"}
local DelayRSlice = {"DelayRSlice", "R 지연 시트", "R延时片"}
local UseRKSDelay = {"UseRKSDelay", "죽일 R을 사용하여", "使用R击杀"}


-- End Language

function OnMenuLoad()
	Config = scriptConfig("Anonymous KogMaw", "APLD KogMaw");
	
	Config:addSubMenu("TargetSelector", "TargetSelector")
	STS:AddToMenu(Config.TargetSelector)
	
	Config:addSubMenu("Language", "Language")
		Config.Language:addParam("Language", "Language", SCRIPT_PARAM_LIST, 1, {"English", "한국어", "中國語"})
	...
	Config:addSubMenu(_KillSteal[_Lg], "KillSteal")
	...
	Config:addSubMenu(_Draw[_Lg], "Draw")
		Config.Draw:addParam("DrawQ", DrawQRange[_Lg], SCRIPT_PARAM_ONOFF, true)
		Config.Draw:addParam("DrawQColor", DrawQColor[_Lg], SCRIPT_PARAM_COLOR, {100, 255, 0, 0})
		Config.Draw:addParam("DrawW", DrawWRange[_Lg], SCRIPT_PARAM_ONOFF, true)
		Config.Draw:addParam("DrawWColor", DrawWColor[_Lg], SCRIPT_PARAM_COLOR, {100, 255, 0, 0})
		Config.Draw:addParam("DrawE", DrawERange[_Lg], SCRIPT_PARAM_ONOFF, true)
		Config.Draw:addParam("DrawEColor", DrawEColor[_Lg], SCRIPT_PARAM_COLOR, {100, 255, 0, 0})
		Config.Draw:addParam("DrawR", DrawRRange[_Lg], SCRIPT_PARAM_ONOFF, true)
		Config.Draw:addParam("DrawRColor", DrawRColor[_Lg], SCRIPT_PARAM_COLOR, {100, 255, 0, 0})
		
			
	Config:addSubMenu(_WSetting[_Lg], "WSetting")
		Config.WSetting:addParam("WAdvance",UseWinAARange[_Lg],SCRIPT_PARAM_ONOFF, false);
		
	
	Config:addSubMenu(_RSetting[_Lg], "RSetting")
		Config.RSetting:addParam("PriorityW", PriorityWtarget[_Lg], SCRIPT_PARAM_ONOFF, true);
		Config.RSetting:addParam("UseRLimit",UseRLimit[_Lg],SCRIPT_PARAM_ONOFF, true);
		Config.RSetting:addParam("LimitRStack",LimitRStack[_Lg], SCRIPT_PARAM_LIST, 5, {"40", "80", "120", "160", "200", "240", "280", "320", "360", "400"});
		Config.RSetting:addParam("UseRDelay", UseRDelay[_Lg], SCRIPT_PARAM_ONOFF, true);
		Config.RSetting:addParam("DelayRChoice", DelayRChoice[_Lg], SCRIPT_PARAM_LIST, 1, { "Random", "Slice",});
		Config.RSetting:addParam("DelayRSlice", DelayRSlice[_Lg], SCRIPT_PARAM_SLICE, 0, 0, 50, 0);
		Config.RSetting:addParam("RKSDelay", UseRKSDelay[_Lg], SCRIPT_PARAM_ONOFF, true)
